// macOS: hint the kernel to prefetch via fcntl(F_RDADVISE, &radvisory), the closest equivalent
// to POSIX_FADV_SEQUENTIAL.
...
// Async-prefetch `[offset, offset+len)` via the same F_RDADVISE primitive as the open-time
// hint, targeted at a moving window.
