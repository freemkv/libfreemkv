...
fn apply_coding_to_track(
    ...
    match coding.and_then(|c| c.field_order()) {
        ...
        // Progressive picture on an interlaced-flagged track: no field order
        // applies (UNDETERMINED, not a guess), and the DECLARED resolution's
        // interlaced flag is corrected — the MEASURED picture is the real fact.
...
mod tests {
    ...
    fn apply_coding_to_track_sets_measured_field_order_never_guesses() {
        ...
        // Progressive picture on an interlaced-flagged track → UNDETERMINED
        // (not faked to TFF/BFF) AND the declared scan type is corrected —
        // measurement supersedes the IFO's declaration.
        let mut t = interlaced_track();
