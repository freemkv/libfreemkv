    ...
    F: FnOnce() -> io::Result<S>,
{
    ...
    // FMTS (AACS 2.1) is checked downstream by resolve_mux_key_map/resolve_fmts_key_map.
        ...
        match reopen() {
            Ok(mut probe) => {
                ...
                let probe_map = match &probe_keys {
                    ...
                    // Non-fatal: a failed key-map resolution leaves the probe without
                    // FMTS ranges, so TrueHD channel correction is skipped (MPLS 7.1/Atmos
                    // stays understated as 5.1). Log the actual error (parity with the sibling re-open failure below), not a bare `.ok()`.
                    .inspect_err(|e| {
                        tracing::debug!(
                            target: "mux",
                            error = %e,
...
// `slots` restricts the search (and order) to candidate pool entries — the
// whole pool for multi-CPS, or just base keys for "which CPS unit is this".
fn pick_pool_slot(
    samples: &[Vec<u8>],
    pool: &[(u32, [u8; 16])],
    slots: &[usize],
...
mod tests {
    ...
    // Decrypt-verdict matrix is owned/tested by `Disc::ensure_decryptable[_keys]`.
