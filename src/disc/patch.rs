        ...
        loop {
            ...
            match read_result {
                ...
                Err(err) => {
                    ...
                    // The retry also failed. Hand off to handle_read_failure
                    // (NOT_READY pause/retry, batch bisect, NonTrimmed mark,
                    // wedge/transport abort). (Scatter-recovery was removed: live
                    // probing proved recalibration does not change a sector's
                    // recovery — the drive's per-sector ECC is media-bound, not
                    // approach-bound — so it only cost time.)
